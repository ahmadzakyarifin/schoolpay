  ...
  <div class="min-h-screen bg-slate-50/50 p-6 md:p-10 font-inter">
    ...
    <div v-else-if="student" class="max-w-6xl mx-auto space-y-8 animate-slide-up">
      ...
      <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
        <div class="white-card p-6 rounded-2xl flex items-center gap-5 shadow-sm border-slate-100">
           <div class="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center"><WalletIcon class="w-6 h-6" /></div>
           <div><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Tagihan</p><p class="text-sm font-black text-slate-700">{{ formatCurrency(financialSummary.total) }}</p></div>
        </div>
        <div class="white-card p-6 rounded-2xl flex items-center gap-5 shadow-sm border-slate-100">
           <div class="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center"><SuccessIcon class="w-6 h-6" /></div>
           <div><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Sudah Dibayar</p><p class="text-sm font-black text-slate-700">{{ formatCurrency(financialSummary.paid) }}</p></div>
        </div>
        <div class="white-card p-6 rounded-2xl flex items-center gap-5 shadow-sm border-slate-100">
           <div :class="['w-12 h-12 rounded-xl flex items-center justify-center', financialSummary.unpaid > 0 ? 'bg-rose-50 text-rose-600' : 'bg-slate-50 text-slate-400']"><AlertIcon class="w-6 h-6" /></div>
           <div><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Sisa Tunggakan</p><p :class="['text-sm font-black', financialSummary.unpaid > 0 ? 'text-rose-600' : 'text-slate-700']">{{ formatCurrency(financialSummary.unpaid) }}</p></div>
        </div>
        <div class="white-card p-6 rounded-2xl flex items-center gap-5 shadow-sm border-slate-100">
           <div class="w-12 h-12 bg-teal-50 text-teal-600 rounded-xl flex items-center justify-center"><WalletIcon class="w-6 h-6" /></div>
           <div><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Saldo Deposit</p><p class="text-sm font-black text-teal-600">{{ formatCurrency(student.deposit_balance || 0) }}</p></div>
        </div>
        <div class="white-card p-6 rounded-2xl flex items-center gap-5 shadow-sm border-slate-100">
           <div class="flex-1">
             <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Status Pelunasan</p>
             <p class="text-sm font-black text-slate-700 mb-2">{{ financialSummary.percentage }}% Terbayar</p>
             <span :class="[
             ...
             ]">{{ financialSummary.status === 'lunas' ? 'Lunas' : financialSummary.status === 'overdue' ? 'Lewat Jatuh Tempo' : financialSummary.status === 'berjalan' ? 'Sedang Berjalan' : 'Belum Ada Tagihan' }}</span>
           </div>
        </div>
      </div>
      ...
      <div class="space-y-6">
        <!-- Tab Navigation -->
        <div class="flex items-center p-1.5 bg-white border border-slate-100 rounded-2xl shadow-sm overflow-x-auto no-scrollbar">
          <button @click="activeTab = 'biodata'" :class="['flex-1 px-8 py-3.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-3 whitespace-nowrap', activeTab === 'biodata' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-indigo-600']">
            <StudentIcon class="w-4 h-4" /> Biodata & Wali
          </button>
          <button @click="activeTab = 'akademik'" :class="['flex-1 px-8 py-3.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-3 whitespace-nowrap', activeTab === 'akademik' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-indigo-600']">
            <GraduationIcon class="w-4 h-4" /> Akademik
          </button>
          <button @click="activeTab = 'keuangan'" :class="['flex-1 px-8 py-3.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-3 whitespace-nowrap', activeTab === 'keuangan' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-indigo-600']">
            <WalletIcon class="w-4 h-4" /> Keuangan
          </button>
        </div>
        ...
        <div class="transition-all duration-500">
          <!-- BIODATA TAB -->
          <div v-if="activeTab === 'biodata'" class="white-card p-10 rounded-2xl shadow-sm border-slate-100 min-h-[650px] animate-fade-in">
          ...
          <div v-if="activeTab === 'akademik'" class="white-card rounded-xl shadow-sm border-slate-100 min-h-[650px] flex flex-col animate-fade-in overflow-hidden">
             <div class="p-10 border-b border-slate-50 flex items-center gap-5 bg-slate-50/20">
                <div class="p-3 bg-indigo-50 text-indigo-600 rounded-xl shadow-sm"><HistoryIcon class="w-5 h-5" /></div>
                <div><h3 class="text-sm font-black text-slate-800 uppercase tracking-widest">Riwayat Progres Akademik</h3><p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Rekaman kenaikan dan perpindahan kelas</p></div>
             </div>
             
             <div class="flex-1 p-10 relative pl-20">
                <div v-if="classHistory.length" class="absolute left-10 top-10 bottom-10 w-1 bg-slate-100 rounded-full"></div>
                
                <div class="space-y-12">
                   <div v-for="(history, index) in paginatedHistory" :key="history.id" class="relative group">
                      <!-- Smaller Grade Circle -->
                      <div :class="['absolute -left-14 w-9 h-9 rounded-xl z-10 flex items-center justify-center border-4 border-white shadow-lg transition-all duration-700', history.is_active ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400']">
                        <span class="text-[10px] font-black">{{ classHistory.length - ((historyPage - 1) * historyLimit + index) }}</span>
                      </div>
                      
                      <div class="flex-1 pb-6 border-b border-slate-50 last:border-0 group-hover:translate-x-2 transition-transform duration-500">
                         <div class="flex items-center justify-between mb-2">
                            <h4 class="text-sm font-black text-slate-700 uppercase tracking-tight">{{ history.class_name }}</h4>
                         ...
                         <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ditempatkan pada: {{ formattedDate(history.created_at) }}</p>
                      </div>
                   </div>
                </div>
             ...
             <div class="px-10 py-6 border-t border-slate-50 flex items-center justify-between bg-slate-50/30">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Halaman {{ historyPage }} dari {{ historyTotalPages }}</p>
                <div class="flex items-center gap-2">
                   <button @click="historyPage--" :disabled="historyPage <= 1" class="p-2.5 border border-slate-200 rounded-xl hover:bg-white hover:shadow-md disabled:opacity-20 disabled:pointer-events-none transition-all"><PrevIcon class="w-4 h-4 text-slate-400" /></button>
                   <button v-for="p in visibleHistoryPages" :key="p" @click="historyPage = p" :class="[p === historyPage ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-200' : 'bg-white text-slate-400 border border-slate-200 hover:border-indigo-300']" class="w-9 h-9 rounded-xl text-[10px] font-black transition-all flex items-center justify-center">{{ p }}</button>
                   <button @click="historyPage++" :disabled="historyPage >= historyTotalPages" class="p-2.5 border border-slate-200 rounded-xl hover:bg-white hover:shadow-md disabled:opacity-20 disabled:pointer-events-none transition-all"><NextIcon class="w-4 h-4 text-slate-400" /></button>
                </div>
             </div>
          </div>
          ...
          <div v-if="activeTab === 'keuangan'" class="white-card rounded-xl shadow-sm border-slate-100 min-h-[650px] flex flex-col overflow-hidden animate-fade-in">
             <div class="px-10 py-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50/20">
                <div class="flex items-center gap-5"><div class="p-3.5 bg-indigo-50 text-indigo-600 rounded-xl shadow-sm"><WalletIcon class="w-5 h-5" /></div><div><h3 class="text-sm font-black text-slate-800 uppercase tracking-widest">Daftar Tagihan Pendidikan</h3><p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Kelola semua kewajiban finansial</p></div></div>
                <!-- Filter Dropdown -->
                <div class="relative min-w-[300px]">
                  <select v-model="selectedYear" class="w-full pl-10 pr-10 py-2.5 bg-slate-50 border border-slate-100 rounded-xl appearance-none focus:bg-white focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 outline-none transition-all font-black text-[10px] text-slate-700 uppercase tracking-widest cursor-pointer">
                    <option value="all">Semua Kelas</option>
                    <option v-for="opt in availableYears" :key="opt.value" :value="opt.value">{{ opt.label }} ({{ opt.academicYear }})</option>
                  </select>
                  <YearIcon class="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" /><ChevronDownIcon class="absolute right-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" />
                </div>
             </div>
             <div class="flex-1 overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead><tr class="bg-slate-50/50"><th class="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tahun Ajaran</th><th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Nama Tagihan</th><th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Nominal</th><th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Status</th><th class="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Jatuh Tempo</th><th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Aksi</th></tr></thead>
                    <tbody class="divide-y divide-slate-50">
                       <tr v-for="bill in paginatedBills" :key="bill.id" class="hover:bg-slate-50/30 transition-colors">
                          <td class="px-10 py-6"><p class="text-[11px] font-black text-slate-700 uppercase">{{ bill.academic_year || '-' }}</p></td>
                          <td class="px-6 py-6"><p class="text-xs font-bold text-slate-500 uppercase tracking-wider truncate">{{ bill.bill_type_name }}</p><p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Periode: {{ bill.period }}</p></td>
                          <td class="px-6 py-6 text-right"><span class="text-xs font-black text-slate-800">{{ formatCurrency(bill.amount) }}</span></td>
                          <td class="px-6 py-6 text-center"><span :class="['px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest border', getStatusColor(bill.status)]">{{ getStatusLabel(bill.status) }}</span></td>
                          <td class="px-10 py-6"><span class="text-[11px] font-bold text-slate-400 uppercase tracking-tight">{{ formattedDate(bill.due_date) }}</span></td>
                          <td class="px-6 py-6 text-center">
                             <button v-if="bill.status !== 'paid'" @click="handlePayManual(bill)" class="px-3 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 border border-emerald-200 rounded-xl text-[8px] font-black uppercase tracking-wider transition-all cursor-pointer">Bayar Manual</button>
                             <span v-else class="text-[9px] font-black text-slate-300 uppercase">-</span>
                          </td>
                       </tr>
                       <tr v-for="i in (billsLimit - paginatedBills.length)" :key="'empty'+i" class="h-[75px]"><td colspan="6"></td></tr>
                   </tbody>
                </table>
             </div>
             <div class="px-10 py-6 border-t border-slate-50 flex items-center justify-between bg-slate-50/30">
                ...
                <div class="flex items-center gap-2">
                   <button @click="billsPage--" :disabled="billsPage <= 1" class="p-2.5 border border-slate-200 rounded-xl hover:bg-white hover:shadow-md disabled:opacity-20 disabled:pointer-events-none transition-all"><PrevIcon class="w-4 h-4 text-slate-400" /></button>
                   <button v-for="p in visibleBillPages" :key="p" @click="billsPage = p" :class="[p === billsPage ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-200' : 'bg-white text-slate-400 border border-slate-200 hover:border-indigo-300']" class="w-10 h-10 rounded-xl text-[11px] font-black transition-all flex items-center justify-center">{{ p }}</button>
                   <button @click="billsPage++" :disabled="billsPage >= billsTotalPages" class="p-2.5 border border-slate-200 rounded-xl hover:bg-white hover:shadow-md disabled:opacity-20 disabled:pointer-events-none transition-all"><NextIcon class="w-4 h-4 text-slate-400" /></button>
                </div>
             </div>
          </div>
        </div>
