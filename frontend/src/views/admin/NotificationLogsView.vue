    ...
    <section v-for="table in channelTables" :key="table.channel" class="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col min-h-[520px] overflow-hidden">
      <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex flex-col sm:items-end gap-3">
          <div :class="[table.channel === 'whatsapp' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-sky-50 text-sky-600 border-sky-100', 'px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-widest flex items-center gap-2 w-fit']">
            <component :is="table.icon" class="w-3.5 h-3.5" />
            {{ table.badge }}
          </div>
          <div class="flex items-center justify-start sm:justify-end gap-2 overflow-x-auto max-w-full pb-1">
            <button
              v-for="statusTab in statusOptions(table.channel)"
              :key="`${table.channel}-${statusTab.value}`"
              @click="selectStatus(table.channel, statusTab.value)"
              class="px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-wider border transition-all cursor-pointer whitespace-nowrap"
              :class="selectedStatusByChannel[table.channel] === statusTab.value ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-600/10' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'"
            >
              {{ statusTab.label }}
            </button>
          </div>
            ...
            <tr v-else v-for="log in logsByChannel[table.channel]" :key="`${table.channel}-${log.id}`" class="hover:bg-slate-50/50 transition-colors group">
              ...
              <td class="py-4 px-6 text-right whitespace-nowrap">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <button v-if="canResend(log)" @click="resendNotification(log)" :disabled="resendingId === log.id" class="px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded-xl transition-all font-black text-[10px] uppercase tracking-wider flex items-center gap-1.5 cursor-pointer disabled:opacity-50">
          ...
          <div class="bg-white w-full max-w-xl relative z-10 rounded-2xl shadow-2xl overflow-hidden animate-scale-in">
            <div class="p-8 space-y-6">
              ...
              <div class="flex items-center gap-3">
                ...
                <button v-if="canResend(selectedLog)" @click="resendNotification(selectedLog); showDetailModal = false" class="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 flex items-center justify-center gap-1.5">
...
const statCards = computed(() => [
  ...
  { label: 'WA Diterima', value: stats.value.DELIVERED || 0, icon: CheckIcon, color: 'bg-sky-50 text-sky-600' },
  { label: 'WA Dibaca', value: stats.value.READ || 0, icon: CheckIcon, color: 'bg-emerald-50 text-emerald-600' },
