    ...
    <div v-if="modelValue" class="absolute top-full left-0 mt-2 w-full max-w-2xl bg-white border border-slate-200 rounded-2xl p-6 shadow-2xl shadow-slate-300/50 z-[100]">
      <div class="flex items-center justify-between mb-6 border-b border-slate-50 pb-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h4 class="font-black text-slate-700 text-xs uppercase tracking-widest">Filter Angkatan</h4>
            <p class="text-[10px] font-bold text-slate-400">Sesuaikan tampilan data angkatan siswa</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="flex flex-col gap-2">
          <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest pl-1">Status Angkatan</label>
          <div class="relative">
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="relative">
            <select v-model="filters.sort" class="w-full py-2.5 px-4 bg-slate-50 border border-slate-100 rounded-xl appearance-none focus:bg-white focus:ring-2 focus:ring-indigo-50 focus:border-indigo-500 outline-none transition-all font-bold text-xs text-slate-700 pr-8 shadow-sm">
              ...
              <option value="created_desc">Data Terbaru</option>
              <option value="created_asc">Data Terlama</option>
            </select>
            <div class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">
              <ChevronDownIcon class="w-3.5 h-3.5" />
            </div>
      ...
      <div class="mt-8 pt-4 border-t border-slate-50 flex justify-end gap-3">
        <button @click="emit('reset')" class="px-6 py-2.5 text-xs font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-colors">Reset</button>
        <button @click="emit('apply')" class="px-8 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-indigo-100 transition-all">Terapkan Filter</button>
      </div>
    </div>
  </transition>
</template>
