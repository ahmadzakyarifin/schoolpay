...
func (h *StudentBillHandler) GetAll(c *gin.Context) {
	...
	if studentIDStr != "" {
		...
		if err != nil || id <= 0 {
			helper.ErrorResponse(c, http.StatusBadRequest, "student_id tidak valid")
			return
		}
...
func (h *StudentBillHandler) Create(c *gin.Context) {
	...
	helper.SuccessResponse(c, http.StatusCreated, "tagihan berhasil dibuat", b)
}
...
func (h *StudentBillHandler) Update(c *gin.Context) {
	...
	helper.SuccessResponse(c, http.StatusOK, "tagihan berhasil diperbarui", b)
}
...
func (h *StudentBillHandler) Delete(c *gin.Context) {
	...
	if strings.TrimSpace(req.Reason) == "" {
		helper.ErrorResponse(c, http.StatusBadRequest, "alasan reset/void tagihan wajib diisi untuk kebutuhan audit")
		return
	}
	...
	helper.SuccessResponse(c, http.StatusOK, "tagihan berhasil dihapus", nil)
}
...
func (h *StudentBillHandler) Generate(c *gin.Context) {
	...
	helper.SuccessResponse(c, http.StatusOK, "tagihan berhasil digenerate", nil)
}
...
func (h *StudentBillHandler) BulkCancel(c *gin.Context) {
	...
	if strings.TrimSpace(req.CustomReason) == "" {
		helper.ErrorResponse(c, http.StatusBadRequest, "alasan penarikan tagihan wajib diisi untuk kebutuhan audit")
		return
	}
	...
	helper.SuccessResponse(c, http.StatusOK, "tagihan berhasil ditarik", nil)
}
...
func (h *StudentBillHandler) Remind(c *gin.Context) {
	...
	helper.SuccessResponse(c, http.StatusOK, "pengingat berhasil dijadwalkan", nil)
}
...
func (h *StudentBillHandler) MarkAsPaidManual(c *gin.Context) {
	...
	if bill.Status == "voided" {
		helper.ErrorResponse(c, http.StatusBadRequest, "tagihan sudah dibatalkan (void)")
		return
	}

	remaining := bill.Amount - bill.TotalPaid
	if remaining <= 0 {
		helper.ErrorResponse(c, http.StatusBadRequest, "tagihan sudah lunas")
		return
	}
	...
	if req.Amount > 0 {
		if req.Amount > remaining {
			helper.ErrorResponse(c, http.StatusBadRequest, "nominal pembayaran melebihi sisa tagihan")
			return
		}
	...
	helper.SuccessResponse(c, http.StatusOK, "pembayaran manual berhasil dicatat", p)
}
